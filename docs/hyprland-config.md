...
3. Verify machine-local files were created from templates:
   - `~/.config/hypr/machine.local.conf`
   - `~/.config/hypr/hypridle-features.local.conf`
   - `~/.config/hypr/workspaces.local.conf` (generated by `switch-workspace-config`)
4. Edit machine-local files as needed for this machine (e.g., disable suspend on laptop)
5. Verify monitor config: run `hyprctl monitors` and check `monitors.conf` uses the
